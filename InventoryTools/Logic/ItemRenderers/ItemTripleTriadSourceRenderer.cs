...
public class ItemTripleTriadSourceRenderer : ItemInfoRenderer<ItemTripleTriadSource>
{
    ...
    public override string SingularName => "九宫幻卡";
    public override string HelpText => "该物品是否从九宫幻卡对局获得？";
    ...
    public override Action<ItemSource> DrawTooltip => (source) =>
    {
        ...
        ImGui.TextUnformatted("对局费用：" + asSource.TripleTriadRow.Base.Fee + SeIconChar.Gil.ToIconString());
        ImGui.TextUnformatted("使用区域规则：" + (asSource.TripleTriadRow.Base.UsesRegionalRules ? "是" : "否"));

        DrawSection("规则：", asSource.TripleTriadRow.Base.TripleTriadRule.Where(c => c.RowId != 0).DistinctBy(c => c.RowId).Select(c => c.Value.Name.ToImGuiString()).ToList());
